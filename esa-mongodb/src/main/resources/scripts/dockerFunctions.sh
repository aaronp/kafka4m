...
export IMAGE_NAME=${IMAGE_NAME:-IMAGE_NAME}
...
dockerRunMongo () {
    ...
    echo "RUNNING: docker run --name $IMAGE_NAME -v "$(pwd)":/data/mount -v "$VOLUME_NAME":/data/db -d mongo:4.0"

    # see "Start a container with a volume" in https://docs.docker.com/storage/volumes/
    docker run --name some-mongo -p 27017:9010 -v "$(pwd)":/data/mount -v "$VOLUME_NAME":/data/db -d mongo:4.0
