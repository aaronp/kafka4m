...
class KafkaRoutesTest extends WordSpec with Matchers with ScalatestRouteTest with FailFastCirceSupport with ScalaFutures {
  ...
  "GET /kafka/topics" should {
    "list the topics" in {
      val routes = new KafkaRoutes(testService, () => ???).routes
  ...
  "GET /kafka/stream" should {
    "open a web socket to pull all data from a query" in {
      ...
      val clientRequest    = UpdateFeedRequest(QueryRequest("a", "b", "c", "d", None, None, StreamStrategy.Latest))
      ...
      val websocketRoute: Route = {
        ...
        new KafkaRoutes(testService, () => flow).routes
      }
