...
class RichKafkaProducer[K, V] private (rootConfig: Config, name: ObjectName, newProducer: Properties => KafkaProducer[K, V])(implicit scheduler: Scheduler)
    extends AutoCloseable
    with RichKafkaProducerMBean
    with StrictLogging {

  private val kafkaConfig                                = rootConfig.getConfig("pipelines.producer")
  private var publisherOpt: Option[KafkaPublisher[K, V]] = None

  private var flushFuture: Cancelable = scheduleFlushes()
  ...
  private def scheduleFlushes(frequency: FiniteDuration = defaultFrequency) = {
    scheduler.scheduleAtFixedRate(frequency, frequency) {
      flush()
    }
  }
  ...
  override def getConfig(): String = {
    rootConfig.getConfig("pipelines").summaryEntries().mkString(";\n")
  }
...
object RichKafkaProducer {
  ...
  private def objectName(): ObjectName = {
    new ObjectName(s"pipelines:name=Publisher-${globalId.incrementAndGet}")
  }
  ...
  def apply[K, V](rootConfig: Config, keySerializer: Serializer[K], valueSerializer: Serializer[V])(implicit scheduler: Scheduler): RichKafkaProducer[K, V] = {
    ...
    def create(props: Properties) = new KafkaProducer[K, V](props, keySerializer, valueSerializer)
