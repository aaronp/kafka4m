...
echo "Starting the REST app on $IPADDR with $@"
...
export JVM_ARGS="$JVM_ARGS -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/app/heapdump/`date`.hprof"
...
java "${JVM_ARGS}" -cp /app/lib/app.jar:/app/config esa.rest.RestServerMain $@